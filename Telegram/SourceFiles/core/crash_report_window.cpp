...
PreLaunchWindow::PreLaunchWindow(QString title) {
	...
	setWindowTitle(title.isEmpty() ? u"Soneta"_q : title);
...
void LastCrashedWindow::saveReport() {
	QString to = QFileDialog::getSaveFileName(0, u"Soneta Crash Report"_q, QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation) + u"/report.sonetacrash"_q, u"Soneta crash report (*.sonetacrash)"_q);
	if (!to.isEmpty()) {
		QFile file(to);
