...
[[nodiscard]] not_null<Window::SessionController *>
FindSessionController(not_null<Ui::RpWidget *> widget) {
...
TopBarSuggestionValue(not_null<Ui::RpWidget *> parent,
  ...
  return [=, outerWrapToggleValue = rpl::duplicate(outerWrapToggleValue),
          ...
          prepareCollapseSnapshot =
              rpl::duplicate(prepareCollapseSnapshot)](auto consumer) {
    ...
    const auto ensureContent = [=]() -> not_null<TopBarSuggestionContent *> {
      if (!state->content) {
        const auto window = FindSessionController(parent);
        state->content = Ui::CreateChild<TopBarSuggestionContent>(parent, [=] {
          return window->isGifPausedAtLeastFor(Window::GifPauseReason::Layer);
        });
        state->content->setCollapseProgress(rpl::duplicate(childListShown));
      }
    ...
    const auto context = TopBarSuggestions::Context{
        ...
        .findController = [=]() -> not_null<Window::SessionController *> {
    ...
    const auto specs =
        lifetime.make_state<std::vector<TopBarSuggestions::Spec>>(
            TopBarSuggestions::AllSpecs());
    ranges::sort(*specs, std::greater<>(), &TopBarSuggestions::Spec::priority);
    ...
    const auto processCurrentSuggestion = [=](auto repeat) -> void {
      ...
      if (winner && winner->dayDependent && state->activeSpecDay &&
          *state->activeSpecDay != currentDay) {
        state->activeSpec = std::nullopt;
      }

      const auto winnerPriority =
          winner ? std::optional<TopBarSuggestions::Priority>(winner->priority)
                 : std::nullopt;
      if (state->activeSpec == winnerPriority && (!winner || state->wrap)) {
        return;
      }
        ...
        auto args = TopBarSuggestions::ActivateArgs{
            ...
            .done =
                [=](not_null<Ui::RpWidget *> widget,
                  ...
                  if (state->wrap && state->wrap->entity() != widget.get()) {
                    state->wrap = nullptr;
                  ...
                  if (!state->wrap) {
                    state->wrap =
                        base::make_unique_q<Ui::SlideWrap<Ui::RpWidget>>(
                            parent, object_ptr<Ui::RpWidget>::fromRaw(widget));
                    state->desiredWrapToggle.force_assign(
                        Toggle{false, anim::type::instant});
                    state->prepareSnapshot = std::move(prepareSnapshot);
                  }
                  state->desiredWrapToggle.force_assign(
      ...
      state->desiredWrapToggle.force_assign(Toggle{false, anim::type::normal});
    ...
    state->desiredWrapToggle.value() | rpl::combine_previous() |
        rpl::filter([=] { return state->wrap != nullptr; }) |
        rpl::on_next(
            [=](Toggle was, Toggle now) {
              state->wrap->toggle(state->outerWrapToggle.current() && now.value,
                                  (was.value == now.value) ? anim::type::instant
                                                           : now.type);
            },
            lifetime);
    ...
    state->outerWrapToggle.value() | rpl::combine_previous() |
        ...
        rpl::on_next(
            [=](bool was, bool now) {
              ...
              state->wrap->toggle(toggle.value && now,
                                  (was == now) ? toggle.type
                                               : anim::type::instant);
            },
            lifetime);

    rpl::merge(session->promoSuggestions().value(),
    ...
    rpl::duplicate(prepareCollapseSnapshot) |
        rpl::on_next(
            [=] {
