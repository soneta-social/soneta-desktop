...
void Activate(ActivateArgs args) {
  ...
  auto ready = crl::guard(content.get(), [=] {
    const auto users =
        promo->knownBirthdaysToday().value_or(std::vector<UserId>());
    if (users.empty()) {
      recompute();
      return;
    }
    ...
    content->setHideCallback([=] {
      ...
      controller->showToast(
          tr::lng_dialogs_suggestions_birthday_contact_dismiss(tr::now));
      recompute();
    });
    auto title = isSingle
                     ? tr::lng_dialogs_suggestions_birthday_contact_title(
                           tr::now, lt_text, {first->shortName()}, tr::rich)
                     : tr::lng_dialogs_suggestions_birthday_contacts_title(
                           tr::now, lt_count, users.size(), tr::rich);
    auto text = isSingle ? tr::lng_dialogs_suggestions_birthday_contact_about(
                               tr::now, TextWithEntities::Simple)
    ...
    if (!isSingle) {
      ...
      const auto rowWidth = rowCount * userpicsSt.size - userpicsSt.shift;
      ...
      widget->paintOn([=](QPainter &p) {
        if (HistoryView::NeedRegenerateUserpics(s->userpics, s->inRow)) {
          HistoryView::GenerateUserpicsInRow(s->userpics, s->inRow,
                                             st::historyCommentsUserpics, 3);
        }
      ...
      const auto fake = Ui::CreateChild<Ui::UserpicButton>(
          content.get(), first, st::uploadUserpicButton);
