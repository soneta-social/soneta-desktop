  ...
  while (!stream.atEnd()) {
    ...
    if (blockId == dbiVersion) {
      ...
      if (memcmp(hashSha1(decrypted.constData(), decrypted.size(), sha1Buffer),
                 dataKey, 16)) {
        LOG(("App Error: bad decrypt key, data from old user config not "
             "decrypted"));
        continue;
      }
...
public:
  CountWaveformTask(not_null<Data::DocumentMedia *> media)
      : _doc(media->owner()), _loc(_doc->location(true)), _data(media->bytes()),
        _wavemax(0) {
...
Window::Theme::Saved readThemeUsingKey(FileKey key) {
  ...
  if (theme.stream.status() == QDataStream::Ok && !chatTheme.isEmpty()) {
    ...
    auto settings =
        base::flat_map<Data::CloudThemeType, Data::CloudTheme::Settings>();
    if (count < 0 || count > 4) {
      return result;
    }
    for (auto i = 0; i != count; ++i) {
      ...
      for (auto j = 0; j != colorsCount; ++j) {
        entry.outgoingMessagesColors.push_back(Serialize::readColor(stream));
      }
      ...
      const auto uncheckedType = static_cast<Data::CloudThemeType>(type);
...
void writeTheme(const Window::Theme::Saved &saved) {
  ...
  if (!object.cloud.emoticon.isEmpty() && !object.cloud.settings.empty()) {
    ...
    stream.setVersion(QDataStream::Qt_5_1);
    stream << object.cloud.emoticon << qint32(object.cloud.settings.size());
    for (const auto &[type, settings] : object.cloud.settings) {
      ...
      stream << (settings.paper ? settings.paper->serialize() : QByteArray());
    }
  }
  ...
  data.stream << object.content << tag << object.pathAbsolute
              ...
              << cache.background << field2 << chatTheme;
