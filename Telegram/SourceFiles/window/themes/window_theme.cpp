...
[[nodiscard]] QImage ReadDefaultImage() {
  return Ui::ReadBackgroundImage(u":/gui/art/background.tgv"_q, QByteArray(),
                                 true)
      .image;
}
...
QByteArray readThemeContent(const QString &path) {
  ...
  if (file.size() > kThemeFileSizeLimit) {
    LOG(("Theme Error: theme file too large: %1 (should be less than 5 MB, got "
         "%2)")
            .arg(path)
            .arg(file.size()));
    return QByteArray();
...
inline uchar readHexUchar(char char1, char char2, bool &error) {
  return ((readHexUchar(char1, error) & 0x0F) << 4) |
         (readHexUchar(char2, error) & 0x0F);
}

bool readNameAndValue(const char *&from, const char *end,
                      QLatin1String *outName, QLatin1String *outValue) {
  ...
  *outName = readName(from, end);
  ...
  if (*from != ':') {
    LOG(("Theme Error: Expected ':' between each name and value in the color "
         "scheme (while reading key '%1')")
            .arg(*outName));
    return false;
  }
  if (!skipWhitespaces(++from, end)) {
  ...
  if (readName(from, end).size() == 0) {
    LOG(("Theme Error: Expected a color value in #rrggbb or #rrggbbaa format "
         "in the color scheme (while reading key '%1')")
            .arg(*outName));
    return false;
  }
...
SetResult setColorSchemeValue(QLatin1String name, QLatin1String value,
  ...
  if (data[0] == '#' && (size == 7 || size == 9)) {
    ...
    if (error) {
      LOG(("Theme Warning: Skipping value '%1: %2' (expected a color value in "
           "#rrggbb or #rrggbbaa or a previously defined key in the color "
           "scheme)")
              .arg(name)
              .arg(value));
      return SetResult::Ok;
  ...
  if (result == style::palette::SetResult::Ok) {
    ...
    LOG(("Theme Warning: Color value appears more than once in the color "
         "scheme (while applying '%1: %2')")
            .arg(name)
            .arg(value));
    return SetResult::Ok;
  } else {
...
bool loadColorScheme(const QByteArray &content,
                     const style::colorizer &colorizer, Instance *out) {
  auto unsupported = QMap<QLatin1String, QLatin1String>();
  return ReadPaletteValues(
      content, [&](QLatin1String name, QLatin1String value) {
...
LoadResult loadBackgroundFromFile(zlib::FileToRead &file, const char *filename,
                                  QByteArray *outBackground) {
  *outBackground = file.readFileContent(filename, zlib::kCaseInsensitive,
                                        kThemeBackgroundSizeLimit);
...
bool loadBackground(zlib::FileToRead &file, QByteArray *outBackground,
                    bool *outTiled) {
...
bool LoadTheme(const QByteArray &content, const style::colorizer &colorizer,
               const std::optional<QByteArray> &editedPalette,
               Cached *cache = nullptr, Instance *out = nullptr) {
  if (content.size() < 4) {
  ...
  if (file.error() == UNZ_OK) {
    ...
    if (schemeContent.isEmpty()) {
      schemeContent =
          file.readFileContent("colors.tdesktop-theme", zlib::kCaseInsensitive,
                               kThemeSchemeSizeLimit);
    }
    if (schemeContent.isEmpty()) {
      file.clearError();
      schemeContent =
          file.readFileContent("colors.tdesktop-palette",
                               zlib::kCaseInsensitive, kThemeSchemeSizeLimit);
    }
    if (file.error() != UNZ_OK) {
      LOG(("Theme Error: could not read 'colors.tdesktop-theme' or "
           "'colors.tdesktop-palette' in the theme file."));
      return false;
    }
    ...
    if (!backgroundContent.isEmpty()) {
      ...
      if (background.isNull()) {
        LOG((
            "Theme Error: could not read background image in the theme file."));
        return false;
      }
      if (colorizer) {
      ...
      if (cache) {
        ...
        if (!background.save(&buffer, "BMP")) {
          LOG(("Theme Error: could not write background image as a BMP to "
               "cache."));
          return false;
        }
    ...
    if (!loadColorScheme(editedPalette.value_or(content), paletteColorizer,
                         out)) {
...
bool InitializeFromCache(const QByteArray &content, const Cached &cache) {
  if (cache.paletteChecksum != style::palette::Checksum()) {
    return false;
  }
  if (cache.contentChecksum !=
      base::crc32(content.constData(), content.size())) {
    return false;
  }
...
[[nodiscard]] std::optional<QByteArray> ReadEditingPalette() {
  ...
  return file.open(QIODevice::ReadOnly) ? std::make_optional(file.readAll())
                                        : std::nullopt;
}
...
bool InitializeFromSaved(Saved &&saved) {
  ...
  if (!editing && !saved.object.cloud.emoticon.isEmpty() &&
...
void ClearApplying() { GlobalApplying = Applying(); }

void ClearEditingPalette() { QFile(EditingPalettePath()).remove(); }

} // namespace

ChatBackground::AdjustableColor::AdjustableColor(style::color data)
    : item(data), original(data->c) {}

// They're duplicated in window_theme_editor_box.cpp:ReplaceAdjustableColors.
ChatBackground::ChatBackground()
    : _adjustableColors({st::msgServiceBg, st::msgServiceBgSelected,
                         st::historyScrollBg, st::historyScrollBgOver,
                         st::historyScrollBarBg, st::historyScrollBarBgOver}) {}

ChatBackground::~ChatBackground() = default;

void ChatBackground::setThemeData(QImage &&themeImage, bool themeTile) {
  _themeImage =
      PostprocessBackgroundImage(std::move(themeImage), Data::ThemeWallPaper());
...
void ChatBackground::start() {
  ...
  _updates.events() | rpl::on_next(
                          [=](const BackgroundUpdate &update) {
                            refreshThemeWatcher();
  ...
  Core::App().domain().activeSessionValue() |
      rpl::filter([=](Main::Session *session) { return session != _session; }) |
      rpl::on_next(
  ...
  rpl::combine(
      ...
      rpl::single(QGuiApplication::styleHints()->colorScheme()) |
          rpl::then(base::qt_signal_producer(QGuiApplication::styleHints(),
                                             &QStyleHints::colorSchemeChanged)),
#endif // Qt >= 6.5.0
      rpl::single(QGuiApplication::palette()) |
          rpl::then(
              base::qt_signal_producer(qApp, &QGuiApplication::paletteChanged))
#if QT_VERSION >= QT_VERSION_CHECK(6, 5, 0)
          ) |
      ...
      rpl::on_next(
          [](bool dark) { Core::App().settings().setSystemDarkMode(dark); },
          _lifetime);
  ...
  rpl::single(QGuiApplication::palette()) |
      ...
      rpl::on_next(
          [=] {
            ...
            if (!settings.systemAccentColorEnabled() || _themeObject.cloud.id ||
                editingTheme()) {
              return;
            }
...
void ChatBackground::refreshThemeWatcher() {
  const auto path = _themeObject.pathAbsolute;
  if (path.isEmpty() || !QFileInfo(path).isNativePath() || editingTheme()) {
    _themeWatcher = nullptr;
  } else if (!_themeWatcher || !_themeWatcher->files().contains(path)) {
    _themeWatcher = std::make_unique<QFileSystemWatcher>(QStringList(path));
    QObject::connect(_themeWatcher.get(), &QFileSystemWatcher::fileChanged,
                     [](const QString &path) {
                       Apply(path);
                       KeepApplied();
                     });
...
void ChatBackground::checkUploadWallPaper() {
  ...
  _wallPaperUploadId =
      FullMsgId(_session->userPeerId(), _session->data().nextLocalMessageId());
  _session->uploader().upload(_wallPaperUploadId, ready);
  if (_wallPaperUploadLifetime) {
    return;
  }
  _wallPaperUploadLifetime =
      ...
      rpl::on_next([=](const Storage::UploadedMedia &data) {
        ...
        _wallPaperRequestId =
            _session->api()
                .request(MTPaccount_UploadWallPaper(
                    MTP_flags(0), data.info.file, MTP_string("image/jpeg"),
                    _paper.mtpSettings()))
                .done([=](const MTPWallPaper &result) {
                  result.match(
                      ...
                      [&](const MTPDwallPaperNoFile &data) {
                        LOG(("API Error: "
                             "Got wallPaperNoFile after "
                             "account.UploadWallPaper."));
                      });
                  if (const auto paper =
                          Data::WallPaper::Create(_session, result)) {
                    setPaper(*paper);
...
void ChatBackground::set(const Data::WallPaper &paper, QImage image) {
  ...
  const auto needResetAdjustable =
      Data::IsDefaultWallPaper(paper) && !Data::IsDefaultWallPaper(_paper) &&
      !nightMode() && _themeObject.pathAbsolute.isEmpty();
  ...
  if (Data::IsThemeWallPaper(_paper)) {
    ...
    if (Data::IsLegacy1DefaultWallPaper(_paper)) {
      ...
      if (scale != 100) {
        image = image.scaledToWidth(style::ConvertScale(image.width(), scale),
    ...
    Local::writeBackground(_paper, ((Data::IsDefaultWallPaper(_paper) ||
                                     Data::IsLegacy1DefaultWallPaper(_paper))
                                        ? QImage()
                                        : image));
    setPreparedAfterPaper(std::move(image));
  }
  Assert(colorForFill() || !_gradient.isNull() ||
         (!_original.isNull() && !_prepared.isNull() &&
...
void ChatBackground::setPreparedAfterPaper(QImage image) {
  ...
  if (_paper.isPattern() && !image.isNull()) {
    if (bgColors.size() < 2) {
      auto prepared = postprocessBackgroundImage(Ui::PreparePatternImage(
          image, bgColors, _paper.gradientRotation(), _paper.patternOpacity()));
      setPrepared(std::move(image), std::move(prepared), QImage());
    } else {
      ...
      setPrepared(image, image, Data::GenerateDitheredGradient(_paper));
    }
  } else if (bgColors.size() == 1) {
    setPrepared(QImage(), QImage(), QImage());
  } else if (!bgColors.empty()) {
    setPrepared(QImage(), QImage(), Data::GenerateDitheredGradient(_paper));
  } else {
    image = postprocessBackgroundImage(std::move(image));
...
void ChatBackground::setPrepared(QImage original, QImage prepared,
                                 QImage gradient) {
  ...
  _imageMonoColor = _gradient.isNull() ? Ui::CalculateImageMonoColor(_prepared)
                                       : std::nullopt;
...
void ChatBackground::adjustPaletteUsingColor(QColor color) {
  ...
  for (const auto &adjustable : _adjustableColors) {
    ...
    adjustable.item.set(adjusted.red(), adjusted.green(), adjusted.blue(),
                        adjusted.alpha());
  }
}

std::optional<QColor> ChatBackground::colorForFill() const {
  return !_prepared.isNull() ? imageMonoColor()
         : (!_gradient.isNull() || _paper.backgroundColors().empty())
             ? std::nullopt
             : std::make_optional(_paper.backgroundColors().front());
}

QImage ChatBackground::gradientForFill() const { return _gradient; }
...
QImage ChatBackground::createCurrentImage() const {
  ...
  auto result = _gradient.scaled(_prepared.size(), Qt::IgnoreAspectRatio,
                                 Qt::SmoothTransformation);
...
bool ChatBackground::tile() const {
  if (!started()) {
    const auto &set =
        nightMode() ? _localStoredTileNightValue : _localStoredTileDayValue;
    if (set.has_value()) {
      return *set;
    }
  }
...
const Object &ChatBackground::themeObject() const { return _themeObject; }
...
void ChatBackground::setTestingTheme(Instance &&theme) {
  ...
  auto switchToThemeBackground =
      !theme.background.isNull() || Data::IsThemeWallPaper(_paper) ||
      (Data::IsDefaultWallPaper(_paper) && !nightMode() &&
  ...
  if (AreTestingTheme() && _editingTheme.has_value()) {
    ...
    if (!Data::IsCustomWallPaper(_paper)) {
      saveForRevert();
      set(Data::details::TestingEditorWallPaper(), base::take(_prepared));
    }
  } else if (switchToThemeBackground) {
    saveForRevert();
    set(Data::details::TestingThemeWallPaper(), std::move(theme.background));
    setTile(theme.tiled);
  } else {
...
bool ChatBackground::isNonDefaultThemeOrBackground() {
  initialRead();
  return nightMode() ? (_themeObject.pathAbsolute != NightThemePath() ||
...
bool ChatBackground::isNonDefaultBackground() {
  initialRead();
  return _themeObject.pathAbsolute.isEmpty() ? !Data::IsDefaultWallPaper(_paper)
                                             : !Data::IsThemeWallPaper(_paper);
}
...
void ChatBackground::writeNewBackgroundSettings() {
  ...
  Local::writeBackground(_paper, ((Data::IsThemeWallPaper(_paper) ||
                                   Data::IsDefaultWallPaper(_paper))
                                      ? QImage()
                                      : _original));
}
...
bool ChatBackground::nightMode() const { return _nightMode; }

void ChatBackground::reapplyWithNightMode(std::optional<QString> themePath,
                                          bool newNightMode) {
  if (!started()) {
  ...
  const auto alreadyOnDisk = [&] {
    ...
    const auto loaded =
        LoadTheme(preview->object.content, ColorizerForTheme(path),
                  std::nullopt, &preview->instance.cached, &preview->instance);
    if (!loaded) {
      return false;
    }
  ...
  if (!alreadyOnDisk) {
    path =
        themePath ? *themePath : (newNightMode ? NightThemePath() : QString());
    ApplyDefaultWithPath(path);
  }
...
bool ChatBackground::nightModeChangeAllowed() const {
  const auto &settings = Core::App().settings();
  const auto allowedToBeAfterChange =
      settings.systemDarkModeEnabled()
...
bool Apply(const QString &filepath, const Data::CloudTheme &cloud) {
...
void KeepFromEditor(const QByteArray &originalContent,
                    const ParsedTheme &originalParsed,
                    const Data::CloudTheme &cloud,
                    const QByteArray &themeContent,
                    const ParsedTheme &themeParsed, const QImage &background) {
  ClearApplying();
  const auto content = themeContent.isEmpty() ? originalContent : themeContent;
  auto saved = Saved();
  ...
  object.cloud = cloud;
  object.content = themeContent.isEmpty() ? originalContent : themeContent;
  object.pathAbsolute = object.pathRelative = CachedThemePath(cloud.documentId);
  Local::writeTheme(saved);
  Background()->keepApplied(saved.object, true);
  Background()->setThemeData(base::duplicate(background), themeParsed.tiled);
...
QString NightThemePath() { return kNightThemeFile.utf16(); }

bool IsNonDefaultBackground() { return Background()->isNonDefaultBackground(); }
...
rpl::producer<bool> IsNightModeValue() {
  ...
  return rpl::single(rpl::empty) | rpl::then(std::move(changes)) |
         rpl::map([=] { return IsNightMode(); }) |
         rpl::distinct_until_changed();
}
...
void ToggleNightMode() { Background()->toggleNightMode(std::nullopt); }

void ToggleNightMode(const QString &path) {
  Background()->toggleNightMode(path);
}

void ToggleNightModeWithConfirmation(not_null<Controller *> window,
                                     Fn<void()> toggle) {
...
void ResetToSomeDefault() {
  Background()->reapplyWithNightMode(
      IsNightMode() ? NightThemePath() : QString(), IsNightMode());
}

bool LoadFromFile(const QString &path, not_null<Instance *> out,
                  Cached *outCache, QByteArray *outContent) {
...
bool LoadFromContent(const QByteArray &content, not_null<Instance *> out,
                     Cached *outCache) {
  return LoadTheme(content, style::colorizer(), std::nullopt, outCache, out);
}

rpl::producer<bool> IsThemeDarkValue() {
  return rpl::single(rpl::empty) | rpl::then(style::PaletteChanged()) |
         rpl::map(
...
bool ReadPaletteValues(
  ...
  if (content.size() > kThemeSchemeSizeLimit) {
    LOG(("Theme Error: color scheme file too large (should be less than 1 MB, "
         "got %2)")
            .arg(content.size()));
    return false;
  }
...
[[nodiscard]] Webview::ThemeParams WebViewParams() {
  ...
  }
  {
    ...
    object.insert("section_separator_color",
                  wrap(QColor(mix(bg.red(), shadow.red()),
...
std::shared_ptr<FilePrepareResult> PrepareWallPaper(MTP::DcId dcId,
                                                    const QImage &image) {
  ...
  const auto push = [&](const char *type, QImage &&image) {
    sizes.push_back(MTP_photoSize(MTP_string(type), MTP_int(image.width()),
                                  MTP_int(image.height()), MTP_int(0)));
    thumbnails.emplace(type[0], PreparedPhotoThumb{.image = std::move(image)});
  };
  push("s", scaled(320));
  ...
  auto attributes = QVector<MTPDocumentAttribute>(
      1, MTP_documentAttributeFilename(MTP_string(filename)));
  attributes.push_back(MTP_documentAttributeImageSize(MTP_int(image.width()),
  ...
  result->document =
      MTP_document(MTP_flags(0), MTP_long(id), MTP_long(0), MTP_bytes(),
                   MTP_int(base::unixtime::now()), MTP_string("image/jpeg"),
                   MTP_long(jpeg.size()), MTP_vector<MTPPhotoSize>(sizes),
                   MTPVector<MTPVideoSize>(), MTP_int(dcId),
