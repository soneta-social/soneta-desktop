...
public:
  ...
  void
  showOrHideBoxOrLayer(std::variant<v::null_t, object_ptr<Ui::BoxContent>,
  ...
  bool showMediaPreview(Data::FileOrigin origin,
                        not_null<DocumentData *> document) const override;
  bool showMediaPreview(Data::FileOrigin origin,
                        not_null<PhotoData *> photo) const override;

  void processChosenSticker(ChatHelpers::FileChosen &&chosen) const override;
...
[[nodiscard]] Ui::CollectibleInfo
Parse(const QString &entity, not_null<PeerData *> owner,
...
void MainWindowShow::showOrHideBoxOrLayer(
    std::variant<v::null_t, object_ptr<Ui::BoxContent>,
                 std::unique_ptr<Ui::LayerWidget>> &&layer,
    Ui::LayerOptions options, anim::type animated) const {
  if (const auto window = _window.get()) {
    window->window().widget()->showOrHideBoxOrLayer(std::move(layer), options,
                                                    animated);
  }
}
...
bool MainWindowShow::valid() const { return !_window.empty(); }

MainWindowShow::operator bool() const { return valid(); }
...
rpl::producer<bool> MainWindowShow::adjustShadowLeft() const {
  ...
  return window->adaptive().value() |
         rpl::map([=] { return !window->adaptive().isOneColumn(); });
}
...
bool MainWindowShow::showMediaPreview(Data::FileOrigin origin,
...
bool IsPaused(not_null<SessionController *> controller, GifPauseReason level) {
  return controller->isGifPausedAtLeastFor(level);
}

Fn<bool()> PausedIn(not_null<SessionController *> controller,
...
DateClickHandler::DateClickHandler(Dialogs::Key chat, QDate date)
    : _chat(chat), _weak(chat.topic()), _date(date) {}

void DateClickHandler::setDate(QDate date) { _date = date; }
...
ForumThreadClickHandler::resolveThread(not_null<HistoryItem *> item) const {
...
SessionNavigation::SessionNavigation(not_null<Main::Session *> session)
    : _session(session), _api(&_session->mtp()) {}

SessionNavigation::~SessionNavigation() = default;

Main::Session &SessionNavigation::session() const { return *_session; }
...
void SessionNavigation::showPeerByLink(const PeerByLinkInfo &info) {
  ...
  if (!info.phone.isEmpty()) {
    ...
    resolveChatLink(info.chatLinkSlug,
                    [=](not_null<PeerData *> peer, TextWithEntities draft) {
        ...
        [=](not_null<PeerData *> peer) {
          if (info.startAutoSubmit) {
            peer->session().api().blockedPeers().unblock(
                peer, [=](bool) { showPeerByLinkResolved(peer, info); }, true);
...
void SessionNavigation::resolvePhone(const QString &phone,
  ...
  _resolveRequestId =
      _api.request(MTPcontacts_ResolvePhone(MTP_string(phone)))
            ...
            if (error.code() == 400) {
              parentController()->show(
                  Ui::MakeInformBox(tr::lng_username_by_phone_not_found(
                      tr::now, lt_phone, Ui::FormatPhone(phone))),
...
void SessionNavigation::resolveChatLink(
  ...
  _api.request(base::take(_resolveRequestId)).cancel();
  _resolveRequestId =
      _api.request(MTPaccount_ResolveBusinessChatLink(MTP_string(slug)))
...
void SessionNavigation::resolveUsername(const QString &username,
                                        Fn<void(not_null<PeerData *>)> done,
  ...
  _resolveRequestId =
      _api.request(MTPcontacts_ResolveUsername(
                       ...
                       MTP_string(username), MTP_string(referral)))
          .done([=](const MTPcontacts_ResolvedPeer &result) {
            resolveDone(result, done);
          })
          .fail([=](const MTP::Error &error) {
...
void SessionNavigation::resolveDone(const MTPcontacts_ResolvedPeer &result,
                                    Fn<void(not_null<PeerData *>)> done) {
  _resolveRequestId = 0;
  parentController()->hideLayer();
...
void SessionNavigation::resolveChannelById(
    ChannelId channelId, Fn<void(not_null<ChannelData *>)> done) {
  ...
  _resolveRequestId =
      _api
          .request(MTPchannels_GetChannels(MTP_vector<MTPInputChannel>(
              1, MTP_inputChannel(MTP_long(channelId.bare), MTP_long(0)))))
...
void SessionNavigation::showMessageByLinkResolved(not_null<HistoryItem *> item,
                                                  const PeerByLinkInfo &info) {
  auto params = SectionShow{SectionShow::Way::Forward};
  params.origin = SectionShow::OriginMessage{info.clickFromMessageId};
...
void SessionNavigation::showPeerByLinkResolved(not_null<PeerData *> peer,
                                               const PeerByLinkInfo &info) {
  auto params = SectionShow{SectionShow::Way::Forward};
  params.origin = SectionShow::OriginMessage{info.clickFromMessageId};
  params.highlight.pollOption = info.pollOption;
  ...
  const auto msgId = useRequestedMessageId  ? info.messageId
                     : info.startAutoSubmit ? ShowAndStartBotMsgId
  ...
  if (const auto threadId = std::get_if<ThreadId>(&replies)) {
    ...
    const auto contextPeer = item ? item->history()->peer : bot;
    const auto action =
  ...
  } else if (bot && (resolveType == ResolveType::AddToGroup ||
                     resolveType == ResolveType::AddToChannel)) {
    const auto scope =
        (resolveType == ResolveType::AddToGroup)
            ? (info.startAdminRights ? Scope::GroupAdmin : Scope::All)
        : (resolveType == ResolveType::AddToChannel) ? Scope::ChannelAdmin
                                                     : Scope::None;
    Assert(scope != Scope::None);

    AddBotToGroupBoxController::Start(parentController(), bot, scope,
                                      info.startToken, info.startAdminRights);
    ...
    if (bot || peer->isChannel()) {
      crl::on_main(this, [=] { showPeerHistory(peer, params); });
    } else {
      showPeerInfo(peer, params);
    }
    ...
    if (!attachBotUsername.isEmpty()) {
      crl::on_main(this, [=] {
        ...
        peer->session().attachWebView().openByUsername(
            parentController(), Api::SendAction(history), attachBotUsername,
            info.attachBotToggleCommand.value_or(QString()),
            info.botAppFullScreen);
      });
    } else if (bot && info.attachBotMainOpen) {
      const auto startCommand = info.attachBotToggleCommand.value_or(QString());
      bot->session().attachWebView().open({
          .bot = bot,
          .context =
              {
      ...
      const auto itemId = info.clickFromMessageId;
      const auto item = _session->data().message(itemId);
      const auto contextPeer = item ? item->history()->peer.get() : nullptr;
      const auto contextUser = contextPeer ? contextPeer->asUser() : nullptr;
      bot->session().attachWebView().open({
          .bot = bot,
          .context =
              {
                  .controller = parentController(),
                  .action = (contextUser
                                 ? Api::SendAction(contextUser->owner().history(
                                       contextUser))
      ...
      crl::on_main(this, [=] {
        ...
        if (historyInNewWindow) {
          const auto window = Core::App().ensureSeparateWindowFor(peer);
...
void SessionNavigation::resolveBoostState(not_null<ChannelData *> channel,
  ...
  _api.request(MTPpremium_GetBoostsStatus(channel->input()))
...
void SessionNavigation::resolveCollectible(PeerId ownerId,
  ...
  _collectibleRequestId =
      _api
          .request(MTPfragment_GetCollectibleInfo(
...
void SessionNavigation::resolveConferenceCall(QString slug,
                                              FullMsgId contextId) {
...
[[nodiscard]] std::vector<not_null<UserData *>>
ExtractParticipantsForInvite(HistoryItem *item) {
  if (!item) {
    return {};
  }
  ...
  const auto add = [&](not_null<PeerData *> peer) {
    if (const auto user = peer->asUser()) {
      if (!user->isSelf() && !ranges::contains(result, not_null(user))) {
...
void SessionNavigation::resolveConferenceCall(QString slug, MsgId inviteMsgId,
                                              FullMsgId contextId) {
  ...
  _conferenceCallRequestId =
      _api.request(MTPphone_GetGroupCall(
                       (inviteMsgId ? MTP_inputGroupCallInviteMessage(
          ...
          .done([=](const MTPphone_GroupCall &result) {
            ...
            result.data().vcall().match(
                [&](const MTPDgroupCall &data) {
                  ...
                  const auto inviter =
                      context ? context->from()->asUser() : nullptr;
                ...
                [&](const MTPDgroupCallDiscarded &data) {
                  if (inviteMsgId) {
                    uiShow()->show(Calls::Group::PrepareCreateCallBox(
                        parentController(), nullptr, inviteMsgId,
                        ExtractParticipantsForInvite(context)));
                  } else {
...
void SessionNavigation::applyBoost(not_null<ChannelData *> channel,
  ...
  _api.request(MTPpremium_GetMyBoosts())
      .done([=](const MTPpremium_MyBoosts &result) {
        ...
        _session->data().processChats(data.vchats());
        const auto slots =
            ParseForChannelBoostSlots(channel, data.vmy_boosts().v);
        if (!slots.free.empty()) {
          ...
          if (!slots.already.empty()) {
            if (const auto receive = BoostsForGift(_session)) {
              ...
              uiShow()->show(Box(Ui::GiftForBoostsBox, name, receive, again));
            } else {
              uiShow()->show(Box(Ui::BoostBoxAlready, channel->isMegagroup()));
            }
            ...
            uiShow()->show(Box(Ui::PremiumForBoostsBox, group, [=] {
              ...
              Settings::ShowPremium(parentController(),
            ...
            uiShow()->show(Box(Ui::GiftedNoBoostsBox, channel->isMegagroup()));
          }
          done({});
        } else {
          const auto weak = std::make_shared<base::weak_qptr<Ui::BoxContent>>();
          const auto reassign = [=](std::vector<int> slots, int groups,
                                    int channels) {
            ...
            const auto callback = [=](Ui::BoostCounters counters) {
              ...
              uiShow()->showToast(tr::lng_boost_reassign_done(
                  tr::now, lt_count, count, lt_channels,
                  (!groups     ? tr::lng_boost_reassign_channels
                   : !channels ? tr::lng_boost_reassign_groups
                               : tr::lng_boost_reassign_mixed)(
                      tr::now, lt_count, groups + channels)));
            };
            applyBoostsChecked(channel, slots, crl::guard(this, callback));
          };
          *weak = uiShow()->show(ReassignBoostsBox(
              channel, slots.other, reassign, [=] { done({}); }));
        }
      })
      .fail([=](const MTP::Error &error) {
...
void SessionNavigation::applyBoostsChecked(not_null<ChannelData *> channel,
                                           std::vector<int> slots,
                                           Fn<void(Ui::BoostCounters)> done) {
  auto mtp = MTP_vector_from_range(
      ranges::views::all(slots) |
      ranges::views::transform([](int slot) { return MTP_int(slot); }));
  _api.request(
          MTPpremium_ApplyBoost(MTP_flags(MTPpremium_ApplyBoost::Flag::f_slots),
                                std::move(mtp), channel->input()))
      .done([=](const MTPpremium_MyBoosts &result) {
        _api.request(MTPpremium_GetBoostsStatus(channel->input()))
            .done([=](const MTPpremium_BoostsStatus &result) {
              channel->updateLevelHint(result.data().vlevel().v);
...
void SessionNavigation::joinVoiceChatFromLink(not_null<PeerData *> peer,
                                              const PeerByLinkInfo &info) {
...
void SessionNavigation::showRepliesForMessage(not_null<History *> history,
                                              MsgId rootId, MsgId commentId,
  ...
  if (_showingRepliesRequestId && _showingRepliesHistory == history.get() &&
  ...
  _showingRepliesRootId = rootId;
  _showingRepliesRequestId =
      _api.request(MTPmessages_GetDiscussionMessage(history->peer->input(),
                                                    MTP_int(rootId)))
          .done([=](const MTPmessages_DiscussionMessage &result) {
            ...
            result.match([&](const MTPDmessages_discussionMessage &data) {
              _session->data().processUsers(data.vusers());
              _session->data().processChats(data.vchats());
              _session->data().processMessages(data.vmessages(),
              ...
              const auto peer =
                  deleted ? history->peer->id : PeerFromMessage(list.front());
              if (!peer || !id) {
                return;
              }
              auto item =
                  deleted ? nullptr : _session->data().message(peer, id);
              if (comments && !item) {
                return;
              }
              ...
              if (deleted || item) {
                using namespace HistoryView;
                auto memento =
                    item ? std::make_shared<ChatMemento>(
                               ChatMemento::Comments(), item, commentId)
...
void SessionNavigation::showPeerInfo(PeerId peerId, const SectionShow &params) {
  showPeerInfo(_session->data().peer(peerId), params);
}

void SessionNavigation::showTopic(not_null<Data::ForumTopic *> topic,
                                  MsgId itemId, const SectionShow &params) {
  return showRepliesForMessage(topic->history(), topic->rootId(), itemId,
                               params);
}

void SessionNavigation::showSublist(not_null<Data::SavedSublist *> sublist,
                                    MsgId itemId, const SectionShow &params) {
  using namespace HistoryView;
  auto memento = std::make_shared<ChatMemento>(
      ChatViewId{
      ...
      itemId, params.highlight);
  showSection(std::move(memento), params);
}

void SessionNavigation::showThread(not_null<Data::Thread *> thread,
                                   MsgId itemId, const SectionShow &params) {
...
void SessionNavigation::showPeerInfo(not_null<PeerData *> peer,
                                     const SectionShow &params) {
...
void SessionNavigation::showPeerInfo(not_null<Data::Thread *> thread,
                                     const SectionShow &params) {
  if (const auto topic = thread->asTopic()) {
...
void SessionNavigation::showPeerHistory(not_null<PeerData *> peer,
                                        const SectionShow &params,
                                        MsgId msgId) {
  showPeerHistory(peer->id, params, msgId);
}

void SessionNavigation::showPeerHistory(not_null<History *> history,
                                        const SectionShow &params,
                                        MsgId msgId) {
  showPeerHistory(history->peer->id, params, msgId);
}

void SessionNavigation::showByInitialId(const SectionShow &params,
                                        MsgId msgId) {
  ...
  case SeparateType::SharedMedia: {
    ...
    showSection(
        (topic ? std::make_shared<Info::Memento>(topic, type)
  ...
  case SeparateType::SavedSublist:
    using namespace HistoryView;
    showSection(std::make_shared<ChatMemento>(ChatViewId{
...
void SessionNavigation::showSettings(Settings::Type type,
                                     const SectionShow &params) {
  showSection(std::make_shared<Info::Memento>(
                  Info::Settings::Tag{_session->user()}, Info::Section(type)),
              params);
}
...
void SessionNavigation::showPollResults(not_null<PollData *> poll,
                                        FullMsgId contextId,
                                        const SectionShow &params) {
...
void SessionNavigation::searchMessages(const QString &query,
                                       Dialogs::Key inChat,
                                       PeerData *searchFrom) {
...
auto SessionNavigation::showToast(TextWithEntities &&text, crl::time duration)
    -> base::weak_ptr<Ui::Toast::Instance> {
...
struct SessionController::CachedThemeKey {
  ...
  friend inline auto operator<=>(const CachedThemeKey &,
...
SessionController::SessionController(not_null<Main::Session *> session,
                                     not_null<Controller *> window)
    : SessionNavigation(session), _window(window),
      _emojiInteractions(
          std::make_unique<ChatHelpers::EmojiInteractions>(session)),
      _chatPreviewManager(std::make_unique<ChatPreviewManager>(this)),
      _isPrimary(window->isPrimary()), _hasDialogs(window->id().hasChatsList()),
      _sendingAnimation(
          std::make_unique<Ui::MessageSendingAnimationController>(this)),
      _tabbedSelector(std::make_unique<ChatHelpers::TabbedSelector>(
          _window->widget(), uiShow(), GifPauseReason::TabbedPanel)),
      ...
      _chatStyle(
          std::make_unique<Ui::ChatStyle>(session->colorIndicesValue())) {
  init();
  ...
  Theme::Background()->updates() |
      rpl::on_next(
          [=](const Theme::BackgroundUpdate &update) {
  ...
  session->data().chatsListChanges() | rpl::filter([=](Data::Folder *folder) {
    return (folder != nullptr) && (folder == _openedFolder.current()) &&
           folder->chatsList()->indexed()->empty() && !folder->storiesCount();
  }) |
      rpl::on_next(
          [=](Data::Folder *folder) {
  ...
  rpl::merge(enoughSpaceForFiltersValue() | rpl::skip(1) | rpl::to_empty,
  ...
  session->downloaderTaskFinished() | rpl::filter([=] {
    return _pendingOpenPhoto.media && _pendingOpenPhoto.media->loaded();
      ...
      rpl::on_next(
          [=] {
            if (_pendingOpenPhoto.media->saveToFile(
                    _pendingOpenPhoto.filepath)) {
  ...
  session->api().globalPrivacy().suggestArchiveAndMute() | rpl::take(1) |
      rpl::on_next(
          [=] {
  ...
  session->downloader().nonPremiumDelays() |
      rpl::on_next(
          [=](DocumentId id) { checkNonPremiumLimitToastDownload(id); },
          _lifetime);

  session->uploader().nonPremiumDelays() |
      rpl::on_next([=](FullMsgId id) { checkNonPremiumLimitToastUpload(id); },
                   _lifetime);
  ...
  crl::on_main(this, [=] {
    ...
    const auto user = session->user();
    session->api()
        .request(MTPpayments_GetSavedStarGifts(
            MTP_flags(MTPpayments_GetSavedStarGifts::Flag::f_exclude_unlimited),
            user->input(), MTP_int(0), MTP_string(QString()), MTP_int(50)))
        .done([=](const MTPpayments_SavedStarGifts &result) {
          ...
          for (const auto &gift : data.vgifts().v) {
            if (auto parsed = Api::FromTL(user, gift)) {
              ...
              if (unique && unique->craftChancePermille > 0 &&
...
bool SessionController::skipNonPremiumLimitToast(bool download) const {
  ...
  const auto delay = session().appConfig().get<int>(
      u"upload_premium_speedup_notify_period"_q, 3600);
...
void SessionController::suggestArchiveAndMute() {
  ...
  _window->show(Box([=](not_null<Ui::GenericBox *> box) {
    ...
    box->addRow(object_ptr<Ui::FlatLabel>(
        box, tr::lng_suggest_hide_new_about(tr::rich), st::boxLabel));
...
SeparateId SessionController::windowId() const { return _window->id(); }

bool SessionController::isPrimary() const { return _isPrimary; }
...
void SessionController::setupShortcuts() {
  ...
  ChatSwitchRequests() | rpl::filter([=](const ChatSwitchRequest &request) {
    return !window().locked() &&
           (_chatSwitchProcess ||
            (request.started && (Core::App().activeWindow() == &window()) &&
             !isLayerShown()));
  }) |
      rpl::on_next(
          [=](const ChatSwitchRequest &request) {
            if (!_chatSwitchProcess) {
              _chatSwitchProcess = std::make_unique<ChatSwitchProcess>(
                  widget()->bodyWidget(), &session(),
                  ...
                  rpl::on_next(
                      [=](not_null<Data::Thread *> thread) {
                        ...
                        if (const auto window =
                                Core::App().separateWindowFor(id)) {
                          window->activate();
                          return;
                        }
                        jumpToChatListEntry(
                            {Dialogs::Key(thread), FullMsgId()});
                      },
  ...
  Requests() | rpl::filter([=] {
    return (Core::App().activeWindow() == &window()) && !isLayerShown() &&
           !window().locked();
  }) |
      rpl::on_next(
          [=](not_null<Request *> request) {
            using C = Command;

            const auto app = &Core::App();
            const auto accountsCount = int(app->domain().accounts().size());
            auto &&accounts = ranges::views::zip(
                kShowAccount, ranges::views::ints(0, accountsCount));
            ...
            request->check(C::SupportHistoryBack) &&
                request->handle([=] { return chatEntryHistoryMove(-1); });
            request->check(C::SupportHistoryForward) &&
                request->handle([=] { return chatEntryHistoryMove(1); });
          },
          lifetime());
}
...
void SessionController::toggleFiltersMenu(bool enabled) {
  if (!_isPrimary || (!enabled == !_filters)) {
    ...
    _filters = std::make_unique<FiltersMenu>(widget()->bodyWidget(), this);
  } else {
    _filters = nullptr;
  }
...
void SessionController::activateFirstChatsFilter() {
  if (_filtersActivated || !isPrimary() ||
...
void SessionController::closeFolder() {
  if (_openedFolder.current() && windowId().type == SeparateType::Archive) {
...
bool SessionController::showForumInDifferentWindow(
    not_null<Data::Forum *> forum, const SectionShow &params,
    MsgId showAtMsgId) {
...
void SessionController::showForum(not_null<Data::Forum *> forum,
                                  const SectionShow &params,
                                  MsgId showAtMsgId) {
  ...
  const auto closeAndShowHistory = [=](bool showOnlyIfEmpty) {
    ...
    const auto showHistory = !now || (!showOnlyIfEmpty && (now == history));
    const auto weak = base::make_weak(this);
    closeForum();
    if (weak && showHistory) {
  ...
  forum->destroyed() |
      rpl::on_next([=] { closeAndShowHistory(false); }, _shownForumLifetime);
  if (!forced) {
...
void SessionController::setupPremiumToast() {
  rpl::combine(
      ...
      session().changes().peerUpdates(Data::PeerUpdate::Flag::FullInfo)) |
      rpl::filter([=] { return session().user()->isFullLoaded(); }) |
      rpl::map([=](bool premium, const auto &) { return premium; }) |
...
bool SessionController::jumpToChatListEntry(Dialogs::RowDescriptor row) {
  if (const auto thread = row.key.thread()) {
    showThread(thread, row.fullId.msg, SectionShow::Way::ClearStack);
    return true;
  }
  return false;
}

void SessionController::setDialogsEntryState(Dialogs::EntryState state) {
...
bool SessionController::switchInlineQuery(Dialogs::EntryState to,
  ...
  MessageCursor cursor = {int(textWithTags.text.size()),
                          int(textWithTags.text.size()), Ui::kQFixedMax};
  ...
  auto draft = std::make_unique<Data::Draft>(textWithTags, to.currentReplyTo,
                                             to.currentSuggest, cursor,
                                             Data::WebPageDraft());
  ...
  if (to.section == Section::Scheduled) {
    ...
    showSection(std::make_shared<HistoryView::ScheduledMemento>(history),
                params);
  } else {
...
bool SessionController::switchInlineQuery(not_null<Data::Thread *> thread,
                                          not_null<UserData *> bot,
                                          const QString &query) {
  const auto entryState = Dialogs::EntryState{
      .key = thread,
      .section = (thread->asTopic() ? Dialogs::EntryState::Section::Replies
...
SessionController::resolveChatNext(Dialogs::RowDescriptor from) const {
  return content()->resolveChatNext(from);
}

Dialogs::RowDescriptor
SessionController::resolveChatPrevious(Dialogs::RowDescriptor from) const {
...
rpl::producer<Dialogs::Key> SessionController::activeChatChanges() const {
  return activeChatEntryChanges() |
         rpl::map(
             [](const Dialogs::RowDescriptor &value) { return value.key; }) |
...
void SessionController::enableGifPauseReason(GifPauseReason reason) {
  if (!(_gifPauseReasons & reason)) {
    auto notify =
        (static_cast<int>(_gifPauseReasons) < static_cast<int>(reason));
...
bool SessionController::isGifPausedAtLeastFor(GifPauseReason reason) const {
  ...
  return (static_cast<int>(_gifPauseReasons) >= 2 * static_cast<int>(reason)) ||
         !widget()->isActive();
}
...
int SessionController::dialogsSmallColumnWidth() const {
  return st::defaultDialogRow.padding.left() + st::defaultDialogRow.photoSize +
         st::defaultDialogRow.padding.left();
}

int SessionController::minimalThreeColumnWidth() const {
  return (_hasDialogs ? st::columnMinimalWidthLeft : 0) +
         st::columnMinimalWidthMain + st::columnMinimalWidthThird;
}
...
auto SessionController::computeColumnLayout() const -> ColumnLayout {
  ...
  auto useOneColumnLayout = [&] {
    auto minimalNormal =
        st::columnMinimalWidthLeft + st::columnMinimalWidthMain;
  ...
  if (useOneColumnLayout()) {
    ...
    auto shrink =
        shrinkDialogsAndThirdColumns(dialogsWidth, thirdWidth, bodyWidth);
...
int SessionController::countDialogsWidthFromRatio(int bodyWidth) const {
  ...
  const auto width =
      bodyWidth * Core::App().settings().dialogsWidthRatio(nochat);
...
SessionController::ShrinkResult SessionController::shrinkDialogsAndThirdColumns(
    int dialogsWidth, int thirdWidth, int bodyWidth) const {
  auto chatWidth = st::columnMinimalWidthMain;
  if (dialogsWidth + thirdWidth + chatWidth <= bodyWidth) {
    return {dialogsWidth, thirdWidth};
  }
  auto thirdWidthNew =
      ((bodyWidth - chatWidth) * thirdWidth) / (dialogsWidth + thirdWidth);
  auto dialogsWidthNew =
      ((bodyWidth - chatWidth) * dialogsWidth) / (dialogsWidth + thirdWidth);
...
bool SessionController::canShowThirdSection() const {
  ...
  auto minimalExtendBy = minimalThreeColumnWidth() - currentLayout.bodyWidth;
...
void SessionController::resizeForThirdSection() {
  ...
  auto tabbedSelectorSectionEnabled = settings.tabbedSelectorSectionEnabled();
  auto thirdSectionInfoEnabled = settings.thirdSectionInfoEnabled();
  ...
  if (extendedBy) {
    ...
    settings.updateDialogsWidthRatio(
        (currentRatio * layout.bodyWidth) / newBodyWidth, nochat);
  }
  auto savedValue = (extendedBy == extendBy) ? -1 : extendedBy;
  settings.setThirdSectionExtendedBy(savedValue);

  settings.setTabbedSelectorSectionEnabled(tabbedSelectorSectionEnabled);
  settings.setThirdSectionInfoEnabled(thirdSectionInfoEnabled);
}
...
void SessionController::closeThirdSection() {
  ...
  if (layout.windowLayout == Adaptive::WindowLayout::ThreeColumn) {
    const auto nochat = !mainSectionShown();
    auto noResize = widget()->isFullScreen() || widget()->isMaximized();
    auto savedValue = settings.thirdSectionExtendedBy();
    auto extendedBy = (savedValue == -1) ? layout.thirdWidth : savedValue;
    auto newBodyWidth =
        noResize ? layout.bodyWidth : (layout.bodyWidth - extendedBy);
    auto currentRatio = settings.dialogsWidthRatio(nochat);
    settings.updateDialogsWidthRatio(
        (currentRatio * layout.bodyWidth) / newBodyWidth, nochat);
    newWindowSize = QSize(widget()->width() + (newBodyWidth - layout.bodyWidth),
                          widget()->height());
  }
  settings.setTabbedSelectorSectionEnabled(false);
  settings.setThirdSectionInfoEnabled(false);
  ...
  if (peer && peer->isChannel() && currentPeer != peer) {
    ...
    if (!clickedChannel->isPublic() && !clickedChannel->amIn() &&
        (!currentPeer->isChannel() ||
         currentPeer->asChannel()->discussionLink() != clickedChannel)) {
      MainWindowShow(this).showToast(
...
void SessionController::showCalendar(ShowCalendarDescriptor &&descriptor) {
  ...
  const auto currentPeerDate = [&] {
    if (topic) {
      ...
      if (const auto migrated =
              history->owner().historyLoaded(history->peer->migrateFrom())) {
  ...
  const auto highlighted = !requestedDate.isNull()     ? requestedDate
                           : !currentPeerDate.isNull() ? currentPeerDate
                                                       : QDate::currentDate();
  ...
  buttonState->disabled.textFg = buttonState->disabled.textFgOver =
      buttonState->disabledFg.color();
  buttonState->disabled.ripple.color = buttonState->disabled.textBgOver =
      buttonState->disabled.textBg;
  const auto selectionChanged = [=](not_null<Ui::CalendarBox *> box,
                                    std::optional<int> selected) {
    if (!selected.has_value()) {
      buttonState->type = ButtonState::Type::None;
      return;
    }
    const auto type = (*selected > 0) ? ButtonState::Type::Active
    ...
    box->clearButtons();
    box->addButton(tr::lng_cancel(), [=] { box->toggleSelectionMode(false); });
    auto text = tr::lng_profile_clear_history();
    const auto button = box->addLeftButton(
        std::move(text),
        [=] {
          ...
          if (!firstDate.isNull()) {
            auto confirm =
                Box<DeleteMessagesBox>(history->peer, firstDate, lastDate);
            confirm->setDeleteConfirmedCallback(
  ...
  const auto searchCalendarResult = [&]() -> SearchCalendarResult {
      ...
      search->monthThumbnails(
          ...
          [=](const std::vector<Api::DayThumbnail> &thumbnails) {
            for (const auto &thumb : thumbnails) {
              set(base::unixtime::parse(thumb.date).date(), thumb.image);
            }
          });
    };
    ...
    if (const auto performJump = descriptor.customJump) {
      customJump = [=](const QDate &d, Fn<void()> close) {
        const auto date = base::unixtime::serialize(QDateTime(d, QTime()));
    ...
    const auto open = [=](not_null<PeerData *> peer, MsgId id) {
      if (const auto strong = weak.get()) {
        if (performJump) {
          ...
          strong->showThread(strongThread, id, SectionShow::Way::Forward);
...
void SessionController::showPassportForm(const Passport::FormRequest &request) {
  _passportForm = std::make_unique<Passport::FormController>(this, request);
  _passportForm->show();
}

void SessionController::clearPassportForm() { _passportForm = nullptr; }

void SessionController::showChooseReportMessages(
    not_null<PeerData *> peer, Data::ReportInput reportInput,
...
void SessionController::showInNewWindow(SeparateId id, MsgId msgId) {
  ...
  const auto fromActive = active.thread() ? (active.thread() == id.thread &&
                                             id.type == SeparateType::Chat)
                                          : false;
...
void SessionController::showPeerHistory(PeerId peerId,
                                        const SectionShow &params,
                                        MsgId msgId) {
  content()->showHistory(peerId, params, msgId);
}

void SessionController::showMessage(not_null<const HistoryItem *> item,
                                    const SectionShow &params) {
  _window->invokeForSessionController(
      &item->history()->session().account(), item->history()->peer,
      [&](not_null<SessionController *> controller) {
        if (item->isScheduled()) {
          controller->showSection(
              std::make_shared<HistoryView::ScheduledMemento>(item->history()),
              params);
...
void SessionController::showSection(std::shared_ptr<SectionMemento> memento,
                                    const SectionShow &params) {
...
void SessionController::showSpecialLayer(object_ptr<Ui::LayerWidget> &&layer,
                                         anim::type animated) {
  widget()->showSpecialLayer(std::move(layer), animated);
}

void SessionController::showLayer(std::unique_ptr<Ui::LayerWidget> &&layer,
                                  Ui::LayerOptions options,
                                  anim::type animated) {
...
bool SessionController::isLayerShown() const { return _window->isLayerShown(); }
...
rpl::producer<bool> SessionController::enoughSpaceForFiltersValue() const {
  return widget()->widthValue() |
         rpl::map([=] { return enoughSpaceForFilters(); }) |
         rpl::distinct_until_changed();
}
...
SessionController::show(object_ptr<Ui::BoxContent> content,
...
bool SessionController::openPhotoExternal(not_null<PhotoData *> photo,
  ...
  const auto filepath =
      FileNameForSave(&session(), tr::lng_save_photo(tr::now),
                      u"JPEG Image (*.jpg);;"_q + FileDialog::AllFilesFilter(),
                      u"photo"_q, u".jpg"_q, false);
  if (media->loaded()) {
...
void SessionController::openPhoto(not_null<PhotoData *> photo,
  ...
  const auto origin =
      item ? Data::FileOrigin(item->fullId()) : Data::FileOrigin();
  if (openPhotoExternal(photo, origin)) {
    return;
  }
  _window->openInMediaView(Media::View::OpenRequest(
      this, photo, item, message.topicRootId, message.monoforumPeerId,
...
void SessionController::openPhoto(not_null<PhotoData *> photo,
                                  not_null<PeerData *> peer) {
  const auto origin =
      peer->isUser() ? Data::FileOrigin(Data::FileOriginUserPhoto(
                           peerToUser(peer->id), photo->id))
...
void SessionController::openDocument(
    not_null<DocumentData *> document, bool showInMediaView,
    MessageContext message, const Data::StoriesContext *stories,
  ...
  if (openSharedStory(item) || openFakeItemStory(message.id, stories)) {
    ...
    if (OptionExternalMediaViewer.value() && !document->isTheme()) {
      ...
      if (filepath.isEmpty()) {
        ...
        DocumentSaveClickHandler::Save(message.id, document,
    ...
    const auto usedTimestamp =
        videoTimestampOverride ? ((*videoTimestampOverride) * crl::time(1000))
        : saved                ? saved
        : timestamp            ? (timestamp * crl::time(1000))
                               : crl::time();
    _window->openInMediaView(OpenRequest(
        this, document, item, message.topicRootId, message.monoforumPeerId,
        false, usedTimestamp, message.showDrawButton));
    return;
  }
  Data::ResolveDocument(this, document, item, message.topicRootId,
                        message.monoforumPeerId, message.showDrawButton);
}
...
bool SessionController::openSharedStory(HistoryItem *item) {
  if (const auto media = item ? item->media() : nullptr) {
    if (const auto storyId = media->storyId()) {
      ...
      if (story) {
        _window->openInMediaView(::Media::View::OpenRequest(
            this, *story, Data::StoriesContext{Data::StoriesContextSingle()}));
      }
      return true;
    }
  }
...
bool SessionController::openFakeItemStory(FullMsgId fakeItemId,
                                          const Data::StoriesContext *stories) {
  if (peerIsChat(fakeItemId.peer) || !IsStoryMsgId(fakeItemId.msg)) {
    return false;
  }
  ...
  if (maybeStory) {
    ...
    const auto context =
        stories ? *stories : StoriesContext{StoriesContextSingle()};
    _window->openInMediaView(::Media::View::OpenRequest(this, story, context));
  }
  return true;
}

auto SessionController::cachedChatThemeValue(const Data::CloudTheme &data,
  ...
  if (paper.isNull() && (!settings->second.paper ||
...
bool SessionController::chatThemeAlreadyCached(const Data::CloudTheme &data,
                                               const Data::WallPaper &paper,
                                               Data::CloudThemeType type) {
  ...
  const auto i = _customChatThemes.find(key);
  return (i != end(_customChatThemes)) && (i->second.theme.lock() != nullptr);
}
...
void SessionController::clearCachedChatThemes() { _customChatThemes.clear(); }

void SessionController::overridePeerTheme(not_null<PeerData *> peer,
...
void SessionController::cacheChatTheme(CachedThemeKey key,
  ...
  Assert((!data.id || (i != end(data.settings))) &&
         (!paper.isNull() || (i->second.paper.has_value() &&
  ...
  auto giftSymbol = findGiftSymbols
                        ? session().data().customEmojiManager().create(
                              data.unique->pattern.document, reportSymbolLoaded,
                              Data::CustomEmojiSizeTag::Large)
                        : nullptr;
  const auto giftId =
      findGiftSymbols ? data.unique->model.document->id : uint64();
  ...
  auto descriptor = Ui::ChatThemeDescriptor{
      .key = key.theme,
      .preparePalette =
          (data.id ? Theme::PreparePaletteCallback(dark, i->second.accentColor)
                   : Theme::PrepareCurrentPaletteCallback()),
      .backgroundData = backgroundData(theme),
      .bubblesData = Theme::PrepareBubblesData(data, type),
      .basedOnDark = dark,
  };
  crl::async([this, descriptor = std::move(descriptor),
              weak = base::make_weak(this)]() mutable {
    crl::on_main(weak, [this, result = std::make_shared<Ui::ChatTheme>(
                                  std::move(descriptor))]() mutable {
...
void SessionController::cacheChatThemeDone(
  ...
  if (media || giftSymbol) {
    if ((!media || media->loaded(true)) &&
      ...
      rpl::merge(session().downloaderTaskFinished(),
                 ((giftSymbol && !giftSymbol->ready())
...
void SessionController::updateCustomThemeBackground(CachedTheme &theme) {
  ...
  if (!strong || (!media && !giftSymbol) || (media && !media->loaded(true)) ||
      (giftSymbol && !giftSymbol->ready())) {
    return;
  }
  ...
  crl::async([=, data = backgroundData(theme, false)] {
    crl::on_main(weak,
                 [=, result = Ui::PrepareBackgroundImage(data)]() mutable {
...
Ui::ChatThemeBackgroundData
SessionController::backgroundData(CachedTheme &theme,
  ...
  const auto darkModeDimming =
      isPattern ? 100 : std::clamp(paper.patternIntensity(), 0, 100);
  return {
      .key = paper.key(),
      .path = paperPath,
      .bytes = paperBytes,
...
void SessionController::openPeerStory(not_null<PeerData *> peer,
                                      StoryId storyId,
  ...
  if (from) {
    ...
    const auto done = crl::guard(
        &_storyOpenGuard, [=] { openPeerStory(peer, storyId, context); });
    stories.resolve({peer->id, storyId}, done);
  }
}

void SessionController::openPeerStories(
    PeerId peerId, std::optional<Data::StorySourcesList> list, bool onlyLive,
    bool afterReload) {
  using namespace Media::View;
  using namespace Data;
  ...
  if (const auto source = stories.source(peerId)) {
    if (const auto idDates = source->toOpen()) {
      ...
      openPeerStory(source->peer, idDates.id,
                    (list ? StoriesContext{*list}
...
HistoryView::PaintContext
SessionController::preparePaintContext(Ui::ChatPaintContextArgs &&args) {
  const auto visibleAreaTopLocal =
      content()->mapFromGlobal(args.visibleAreaPositionGlobal).y();
  const auto area = QRect(0, args.visibleAreaTop, args.visibleAreaWidth,
                          args.visibleAreaHeight);
  const auto viewport = QRect(0, args.visibleAreaTop - visibleAreaTopLocal,
                              args.visibleAreaWidth, content()->height());
  return args.theme->preparePaintContext(
      _chatStyle.get(), viewport, area, args.clip,
      isGifPausedAtLeastFor(GifPauseReason::Any));
}

void SessionController::setPremiumRef(const QString &ref) { _premiumRef = ref; }

QString SessionController::premiumRef() const { return _premiumRef; }

bool SessionController::showChatPreview(
    Dialogs::RowDescriptor row, Fn<void(bool shown)> callback,
    QPointer<QWidget> parentOverride, std::optional<QPoint> positionOverride) {
  return _chatPreviewManager->show(std::move(row), std::move(callback),
                                   std::move(parentOverride), positionOverride);
}

bool SessionController::scheduleChatPreview(
    Dialogs::RowDescriptor row, Fn<void(bool shown)> callback,
    QPointer<QWidget> parentOverride, std::optional<QPoint> positionOverride) {
  return _chatPreviewManager->schedule(std::move(row), std::move(callback),
...
void SessionController::saveSubsectionTabs(
  ...
  _savedSubsectionTabs->removeRequests() |
      rpl::on_next([=] { _savedSubsectionTabs = nullptr; },
                   _savedSubsectionTabsLifetime);
}

auto SessionController::restoreSubsectionTabsFor(
    not_null<Ui::RpWidget *> parent, not_null<Data::Thread *> thread)
...
void SessionController::showStarGiftAuction(const QString &slug) {
  ...
  const auto requesting =
      _starGiftAuctionLifetime.make_state<base::has_weak_ptr>();
...
void SessionController::showCloudPassword(const QString &highlight) {
  ...
  _showCloudPasswordLifetime =
      rpl::single(PasswordState::Loading) |
      rpl::then(session().api().cloudPassword().state() |
                rpl::map([](const Core::CloudPasswordState &state) {
                  return (!state.unconfirmedPattern.isEmpty())
                         ...
                         : state.hasPassword ? PasswordState::On
                                             : PasswordState::Off;
                })) |
      rpl::distinct_until_changed() |
      rpl::filter(rpl::mappers::_1 != PasswordState::Loading) | rpl::take(1) |
      rpl::on_next([=](PasswordState state) {
...
void SessionController::checkHighlightControl(const QString &id,
                                              QWidget *widget,
...
bool CheckAndJumpToNearChatsFilter(not_null<SessionController *> controller,
                                   bool isNext, bool jump) {
  const auto id = controller->activeChatsFilterCurrent();
  const auto session = &controller->session();
  const auto list = &session->data().chatsFilters().list();
  const auto index =
      int(ranges::find(*list, id, &Data::ChatFilter::id) - begin(*list));
  ...
  if (jump) {
    controller->setActiveChatsFilter((changed >= 0) ? (*list)[changed].id()
                                                    : 0);
  }
  return true;
}
